...
path = r"./left/18.png"
temp_L = cv2.imread(r"./left/temp.png", cv2.IMREAD_GRAYSCALE)
THRESHOLD_VALUE = 13
A = -2
HEIGHT_AREA = 38
HEIGHT_3RD_AREA = 43
...
#     # Sửa tọa độ để cắt và khoanh từ bên trái của white_region
#     third_crop_top_left = (white_top_left[0], white_top_left[1])  # Bắt đầu từ bên trái của white_region
#     third_crop_bottom_right = (white_top_left[0] + WIDTH_3RD_AREA, white_bottom_right[1])
...
def crop_regions(img_display, origin, top_left, bottom_right, red_top_left, red_bottom_right):
    # Sao chép ảnh gốc để không ảnh hưởng đến origin
    img_display = origin.copy()
    ...
    # Cắt và khoanh vùng thứ 3 từ bên trái của white_region
    third_crop_top_left = (white_top_left[0], white_top_left[1])
    third_crop_bottom_right = (white_top_left[0] + WIDTH_3RD_AREA, white_bottom_right[1])
    area_3rd_crop = crop_by_regions(white_region, WIDTH_3RD_AREA)
    ...
    # Vẽ các hình chữ nhật lên img_display
    cv2.rectangle(img_display, red_top_left, red_bottom_right, white_color, LINE_THINESS)  # Vùng đỏ đầy đủ
    cv2.rectangle(img_display, red_left_top, red_left_bottom, toggle_rectangle_draw(left_percent, CONF_2), LINE_THINESS)  # Nửa trái vùng đỏ
    cv2.rectangle(img_display, red_right_top, red_right_bottom, toggle_rectangle_draw(right_percent, CONF_3), LINE_THINESS)  # Nửa phải vùng đỏ
    cv2.rectangle(img_display, third_crop_top_left, third_crop_bottom_right, toggle_rectangle_draw(percent_3rd, CONF_4), LINE_THINESS)  # Vùng thứ 3
    ...
    # Hiển thị ảnh Result với các hình chữ nhật
    cv2.imshow("Result", img_display)

def update_threshold(val):
    global THRESHOLD_VALUE, img_display, temp_L, origin, A
    THRESHOLD_VALUE = val
    img_display = origin.copy()  # Reset img_display
    pt, top_left, bottom_right, red_top_left, red_bottom_right = process_image(img_display, temp_L, CONF_TEMPLATE, THRESHOLD_VALUE)
    print(f"Threshold: {THRESHOLD_VALUE}")
    ...
    if pt:
        ...
        print('Khong tim thay template')
        cv2.imshow("Result", img_display)  # Vẫn hiển thị ảnh nếu không tìm thấy template
...
if pt:
    ...
    cv2.imshow("Result", img_display)  # Hiển thị ảnh mặc định nếu không tìm thấy template
